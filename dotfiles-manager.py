...
class DotfilesManager:
    ...
    def _is_valid_package(self, package: str) -> bool:
        ...
        if not package_dir.exists() or not package_dir.is_dir():
            self._print_error(f"Package '{package}' not found in {self.config.dotfiles_dir}")
            return False
        return True
    ...
    def _show_preview(self, operation: str, packages: List[str], dry_run_results: Dict[str, Tuple[bool, str]]):
        ...
        if total_conflicts > 0:
            self._print_warning(f"Conflicts: {total_conflicts} (backups will be created)")
    ...
    def adopt(self, packages: List[str], git_commit: bool = True):
        ...
        if total_conflicts == 0:
            self._print_warning("No conflicts to adopt. Use 'install' to create symlinks.")
            return

        self._print_warning(f"\nTotal files to adopt: {total_conflicts}")
        self._print_warning("WARNING: Files in dotfiles repository will be OVERWRITTEN!")
    ...
    def _handle_sudo_package(self, package: str, operation: str):
        ...
        if operation == 'install':
            ...
            if logid_dst.exists():
                try:
                    subprocess.run(['sudo', 'cp', str(logid_dst),
                                  str(logid_dst) + '.backup'], check=True)
            ...
            # Copy with sudo
            try:
                subprocess.run(['sudo', 'cp', str(logid_src), str(logid_dst)], check=True)
                subprocess.run(['sudo', 'chown', 'root:root', str(logid_dst)], check=True)
                self._print_success(f"System config installed: {logid_dst}")
                self._log('SUCCESS', f"Sudo package 'config' installed to /etc/")
            except subprocess.CalledProcessError as e:
            ...
            if logid_dst.exists():
                if self._confirm_action(f"Remove {logid_dst}? (requires sudo)"):
                    try:
                        ...
                        self._log('SUCCESS', f"Sudo package 'config' uninstalled from /etc/")
                    except subprocess.CalledProcessError as e:
    ...
    def _git_create_commit(self, packages: str):
        ...
        try:
            ...
            subprocess.run(['git', 'add', '-A'],
                         cwd=str(self.config.dotfiles_dir), check=True)
            ...
            for rel_path in sorted(package_files):
                ...
                if target_file.is_symlink():
                    ...
                    if link_target == dotfile.resolve():
                        ...
                        print(f"  {Colors.YELLOW}⚠{Colors.RESET} {rel_path} (wrong target)")
                elif target_file.exists():
                    print(f"  {Colors.RED}✗{Colors.RESET} {rel_path} (file exists, not symlink)")
                else:
    ...
    def list_packages(self):
        ...
        packages = [d.name for d in self.config.dotfiles_dir.iterdir()
...
Examples:
  ...
  %(prog)s list
    ...
    install_parser.add_argument('-n', '--dry-run', action='store_true', help='Simulate only')
    ...
    uninstall_parser.add_argument('-a', '--all', action='store_true', help='All packages')
    uninstall_parser.add_argument('-n', '--dry-run', action='store_true', help='Simulate only')
    ...
    restow_parser.add_argument('-n', '--dry-run', action='store_true', help='Simulate only')
    restow_parser.add_argument('-v', '--verbose', action='store_true', help='Verbose output')
    ...
    adopt_parser.add_argument('-v', '--verbose', action='store_true', help='Verbose output')
